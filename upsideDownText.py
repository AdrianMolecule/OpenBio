...
class UpsideDownText(tk.Text):
    ...
    def _draw_rotated_text(self, text, x, y):
        ...
        self.canvas.create_text(x, y, text=text, font=self.cget("font"), anchor=tk.CENTER, angle=180)
    ...
    def insert(self, index, text, *args):
        ...
        self._draw_rotated_text(self.get("1.0", tk.END).strip(), 0, 0)
...
upside_down_text = UpsideDownText(root, width=0, height=0, wrap=tk.WORD, bd=1, font=("Arial", 10),relief="solid", highlightbackground="red")
upside_down_text.insert(tk.END, "HELLO WORLD AGAIN AND AGAIN")  # Insert the text
upside_down_text.pack()
...
# canvas_window = canvas.create_window(x,y+20,width=x + length*baseRectangleSymbolXPixelSize,height=y + 3, window=text_widget, anchor="nw") 
